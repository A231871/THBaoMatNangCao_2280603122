...
# --- FIX 1: Corrected the class name import ---
from ui.rsa import Ui_RSACipherWindow 

# A base URL makes it easier to change the server address later
API_BASE_URL = "http://127.0.0.1:5000/api/rsa"

class MyApp(QMainWindow):
    def __init__(self):
        super().__init__()
        # --- FIX 1: Use the correct UI class ---
        self.ui = Ui_RSACipherWindow()
        self.ui.setupUi(self)

        # --- FIX 2: Use the correct widget names for connecting signals ---
    ...
    def call_api_encrypt(self):
        # --- FIX 2: Use the correct widget name to get text ---
        message = self.ui.textEditPlain.toPlainText()
        if not message:
            self.show_message("Plain text cannot be empty.", QMessageBox.Warning)
            return
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # --- FIX 2: Use the correct widget name to set text ---
                self.ui.textEditCipher.setText(data["encrypted_message"])
    ...
    def call_api_decrypt(self):
        # --- FIX 2: Use the correct widget name to get text ---
        ciphertext = self.ui.textEditCipher.toPlainText()
        if not ciphertext:
            self.show_message("Cipher text cannot be empty.", QMessageBox.Warning)
            return
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # --- FIX 2: Use the correct widget name to set text ---
                self.ui.textEditPlain.setText(data["decrypted_message"])
    ...
    def call_api_sign(self):
        # --- FIX 2: Use the correct widget name to get text ---
        message = self.ui.textEditInfo.toPlainText()
        if not message:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # --- FIX 2: Use the correct widget name to set text ---
                self.ui.textEditSignature.setText(data["signature"])
    ...
    def call_api_verify(self):
        # --- FIX 2: Use the correct widget names to get text ---
        message = self.ui.textEditInfo.toPlainText()
        signature = self.ui.textEditSignature.toPlainText()
